...
def getBox(img, i, j): 
	...
	cellW = width/9
	cellH = height/9
	box = outerBox[int(cellH*i + padding):int(cellH*(i+1) - padding), int(cellW*j + padding):int(cellW*(j+1) - padding)]
	return box
	...
	for i in range(0, 9):
		for j in range(0, 9):
			if(originBoard[i][j] == 0):
				...
				cv.putText(img, text, (int(textX), int(textY) ), font, 1, (255, 0, 0), 2)
...
if __name__ == '__main__':
	...
	if(sudukoSolverFinal.solve(resBoard) == True):
		print('result:')
		print(resBoard)
